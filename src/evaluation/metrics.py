...
def evaluate_policy(
    ...
    Metrics computed per episode:
        - Mean reward
        - Attack success rate
        - Detection delay (steps between first high-threat and first response)
        - False positive rate
        - Service downtime
        - Cost per episode
    """
    env_kw = dict(env_kwargs or {})
    env = IntrusionEnv(attack_mode=attack_mode, intensity=intensity, **env_kw)
    ...
    for ep in range(n_episodes):
        ...
        while not done:
            ...
            if info["threat_level"] > IntrusionEnv.HIGH_THREAT_THRESHOLD:
                high_threat_steps += 1
            ...
            if info["threat_level"] < IntrusionEnv.LOW_THREAT_THRESHOLD and action > 0:
        ...
        if first_high_step >= 0 and first_action_step >= 0:
            ...
            em.detection_delay = total_steps - first_high_step

        # FPR
        low_threat_steps = sum(
            1 for t in em.threat_levels if t < IntrusionEnv.LOW_THREAT_THRESHOLD
        )
        em.false_positive_rate = em.false_positive_count / max(low_threat_steps, 1)
...
def multi_seed_evaluate(
    ...
    Returns dict with mean/std/CI across seeds for all metrics.
    """
    if seeds is None:
...
def run_ood_tests(
    ...
    for scenario_name, scenario_cfg in OOD_SCENARIOS.items():
        cfg = dict(scenario_cfg)  # copy to avoid mutation
        attack_mode = cfg.pop("attack_mode", "brute_force")
        intensity = cfg.pop("intensity", "medium")
        env_kwargs = cfg

        result = evaluate_policy(
            policy,
            f"{policy_name}_{scenario_name}",
            attack_mode=attack_mode,
            intensity=intensity,
            n_episodes=n_episodes,
            seed=seed,
...
def _bootstrap_ci(
    ...
    if n < 2:
        ...
        return (m, m)
    boot_means = np.array(
        [float(np.mean(rng.choice(data, size=n, replace=True))) for _ in range(n_bootstrap)]
    )
